...
def complete_task():
    ...
    if not tasks:
        return
    try:
        task_number=int(input("Enter task number to be completed: "))
        if task_number<1 or task_number>len(tasks):
            print("Invalid task number.")
            return
        task_to_be_completed=tasks[task_number-1]
        task_to_be_completed["completed"]=True
        print(f"Task '{task_to_be_completed["title"]}' marked as completed!")
